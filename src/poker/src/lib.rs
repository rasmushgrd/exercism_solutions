...
impl Card {
    fn new(value: &str, suit: char) -> Self {
        Self {
            rank: Card::get_rank(value).unwrap(),
            suit: Card::get_suit(suit).unwrap(),
        }
    }

    fn get_rank(value: &str) -> Option<u8> {
        match value {
            "A" => Some(14),
            "K" => Some(13),
            "Q" => Some(12),
    ...
    fn get_suit(suit: char) -> Option<CardSuit> {
        match suit {
...
impl Hand {
    ...
    fn is_full(&self) -> bool {
        if self.cards.len() >= 5 { true } else { false }
    }
